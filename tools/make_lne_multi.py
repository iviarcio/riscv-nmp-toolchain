...
def main(firmware, parameter, runscript, inout_info, mean, scale, input_type, data_format):
# data for .lne schema
	lne_version = 3
	fsize = 0
	psize = 0
	...
	#	print("layer size : ", LAYERS[i]['size'])
...
#	bsize = bsize * 2
 ...
 # TODO : 64Mb
	...
	for i in range(input_num):
		...
		print("input shape: ", input_shape[i])
		print("input Q: ", inputq[i])
	...
	for i in range(output_num):
		...
		print("output shape: ", output_shape[i])
		print("output Q: ", outputq[i])
	...
	tflite.SubGraph.SubGraphStartTensorsVector(builder, 2 + input_num + output_num)
	...
	lne_tensor = builder.EndVector(2 + input_num + output_num)


	# sub operator inputs / outputs
	tflite.SubGraph.SubGraphStartInputsVector(builder, 2 + input_num)
	...
	tflite.OneShotOptions.OneShotOptionsAddInputQ(builder, inputq_multi)
	tflite.OneShotOptions.OneShotOptionsAddOutputQ(builder, outputq_multi)
	...
	#TODO: I dont know.. buffer free / input / output
	buf_output = []
	for i in range(output_num):
		tflite.Buffer.BufferStart(builder)
	...
	tflite.Model.ModelStartBuffersVector(builder, 3 + input_num + output_num)
	for i in range(output_num):
	...
	lne_buffers = builder.EndVector(3 + input_num + output_num)
...
if __name__ == '__main__':
	...
	parser.add_argument('-i', help="input/output info.txt(ex> inout_info.txt)")
	...
	main(args.f, args.p, args.r, args.i, args.m, args.s, args.it, args.df)
